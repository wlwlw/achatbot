...
p.add_argument('--output', '-o', help='Output to file. Otherwise show.', 
               nargs='?')
p.add_argument('--mode', '-m', help='Set mode to IPC stat IPC, cache to stat cache-misses.', 
               nargs='?')
args = p.parse_args()
...
for row in rc:
    ...
    ## modify event statistic to fullfill the requirement
    while(1):
        ...
        old_ts = timestamps[event][-1] if(len(timestamps[event])>0) else 0
        if(float(ts)!=old_ts):
            ...
            try:
                ...
                val = value[event][-1]
                value[event].append(val)
        else:
            try:
                value[event][-1]=value[event][-1]+float(val.replace("%",""))
            except ValueError:
                val = value[event][-1]

        if(event=="instructions"):
            val = str(float(val)/value["cycles"][-1])
            event = "IPC"
            continue
        if(event=="cache-misses"):
            val = str(1000*float(val)/value["instructions"][-1])
            event = "L2-MPKI"
            continue
        if("misses" in event):
            val = str(1000*float(val)/value["instructions"][-1])
            if("L1" in event):
                ...
                event = event.replace("misses", "MPKI")
            continue

        break

# set assigned to appropriate targets
for key in assigned.keys():
    if(args.mode == "IPC" and "IPC" in key): continue
    if(args.mode == "MPKI" and ("MPKI" in key)): continue
    if(args.mode == "L1" and ("L1-MPKI" in key)): continue
    if(not args.mode): continue
    del assigned[key]
